...
class MisReportAccountCoverageCheck(models.TransientModel):
    ...
    def action_check(self):
        ...
        # account.account.code is company-dependent since 18.0 (backed by
        # code_mapping_ids, resolved against self.env.company): search it
        # under one of the instance's own companies, not whatever company
        # happens to be active for the current user.
        account_model = self.env[
            report.account_model or "account.account"
        ].with_company(instance.query_company_ids[:1])
        ...
        range_accounts = account_model.search(
            ...
            + [("company_ids", "in", instance.query_company_ids.ids)]
        )
