...
class TestMisReportAccountCoverageCheck(common.TransactionCase):
    ...
    def setUpClass(cls):
        ...
        # account.account.code is company-dependent since 18.0: make this
        # the active company so every account/journal/move created below
        # resolves its code against it, not the default demo company.
        cls.env = cls.env(
        ...
        cls.env["mis.report.kpi"].create(
            {
                ...
                "name": "balance",
                "description": "Balance",
                "expression_ids": [Command.create({"name": "balp[400AR,700IN]"})],
            }
        )
        cls.instance = cls.env["mis.report.instance"].create(
            {
                ...
                "period_ids": [Command.create({"name": "Default"})],
            }
        )
